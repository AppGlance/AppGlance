...
public enum Signal {
    ...
    /// The presence ping sent every `heartbeatInterval` while the app is in the foreground.
    /// Powers "active right now" and session length; never billable, never shown as an event.
